...
function showCard(tileIndex, {canAuction=false}={}) {
  ...
  if (t.type === 'prop') {
    ...
    const isNoRentProp = ['utility','rail','bus','ferry','air','casino_bj','casino_roulette','fiore'].includes(t.subtype);

    if (cardPriceRow) cardPriceRow.style.display = 'flex';
    if (cardRentRow)  cardRentRow.style.display  = isNoRentProp ? 'none' : 'flex';
    ...
    if (startBtn) startBtn.style.display = (canAuction && t.owner === null) ? 'inline-block' : 'none';

    if (isNoRentProp) {
      const defs = { casino_bj:'Kartetara olgetan', casino_roulette:'TODO AL NEGRO COMO WILLIAMS', fiore:'Negocio de Txirloras: al final turno se paga las bihergiñes.' };
      bankWarn.className = 'muted';
      bankWarn.textContent = defs[t.subtype];
    } else {
      bankWarn.textContent = t.mortgaged ? 'Hipotecada: el alquiler lo cobra el Estado.' : '';
    }
