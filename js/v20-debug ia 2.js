...
(function(){
  ...
  \1// NUEVO — pestaña Roles + sección
const tabRoles = el('button', { textContent:'Roles' });
tabRoles.style.cssText = 'margin-right:6px;padding:4px 8px;border:1px solid #ddd;background:#f8fafc;border-radius:8px;cursor:pointer';
const secRoles = el('div'); secRoles.style.display = 'none';

tabs.append(tabRoles);           // añade la pestaña al contenedor
card.appendChild(secRoles);      // añade la sección al card (después de secLog también vale)
...
btnEstadoBid.style.cssText = 'padding:4px 8px;border:1px solid #ddd;background:#fee;border-radius:8px;cursor:pointer';
...
tabs.append(tabState, tabLog);
  ...
  if (which==='log'){
    secState.style.display='none'; secLog.style.display='block'; secRoles.style.display='none';
    tabLog.style.background='#fff'; tabState.style.background='#f8fafc'; tabRoles.style.background='#f8fafc';
  } else if (which==='roles'){
    secState.style.display='none'; secLog.style.display='none'; secRoles.style.display='block';
    tabRoles.style.background='#fff'; tabState.style.background='#f8fafc'; tabLog.style.background='#f8fafc';
    renderRoles();    // ← refresca al abrir
  } else {
    secState.style.display='block'; secLog.style.display='none'; secRoles.style.display='none';
    tabState.style.background='#fff'; tabLog.style.background='#f8fafc'; tabRoles.style.background='#f8fafc';
  }
}
    else { secState.style.display='block'; secLog.style.display='none'; tabState.style.background='#fff'; tabLog.style.background='#f8fafc'; }
  }
  \1tabRoles.onclick = () => selectTab('roles');
toggleBtn.onclick = () => {
  ...
  if (DBG.enabled) {
    render();
    if (typeof secRoles !== 'undefined' && secRoles.style.display === 'block') renderRoles();
  }
}, 500);
