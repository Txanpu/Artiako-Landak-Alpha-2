...
const V13_COLORS = {
  ...
  util:'#64748b', rail:'#94a3b8', ferry:'#60a5fa', air:'#0ea5e9',
...
function createTileElement(tile, index){
  ...
  el.addEventListener('click', ()=>{ if(tile&&tile.type==='prop'&&typeof window.showCard==='function'){ window.showCard(index,{canAuction:false}); } });
...
function refreshTile(i){
  ...
  else if (t.type==='jail') left.textContent='Cárcel';
  ...
  if(t.type==='prop'){
    ...
    if(t.subtype){ const b=document.createElement('span'); b.className='badge'; b.textContent=({utility:'⚙️ Utility',rail:'🚇 Metro',ferry:'⛴️ Ferry',air:'✈️ Aeropuerto'}[t.subtype]||t.subtype); badges.appendChild(b); }
  }

  const old=el.querySelector('.chips'); if(old) old.remove();
  if(V13.state && Array.isArray(V13.state.players)){
    const here=V13.state.players.filter(p=>p.alive&&p.pos===i);
    if(here.length){
      const chips=document.createElement('div'); chips.className='chips';
      here.forEach(p=>{ const c=document.createElement('div'); c.className=`chip p${p.id}`; c.textContent=(p.id+1 ?? p.id); chips.appendChild(c); });
      el.appendChild(chips);
    }
  }
}
function refreshTiles(){ for(let i=0;i<V13.tiles.length;i++) refreshTile(i); }
...
document.addEventListener('DOMContentLoaded', ()=>{
  const tiles=window.TILES||[]; const state=window.state||null;
